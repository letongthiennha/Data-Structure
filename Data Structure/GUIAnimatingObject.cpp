...
void GUIAnimatingObject::update() {
	...
	if (m_animationQueue.empty()) return;
	//Animation Done
	if (isSameColor(m_state->getMainColor() ,m_animationQueue.front().m_targetState->getMainColor()) &&
		isSamePosition(m_state->getPosition(), m_animationQueue.front().m_targetState->getPosition())) {
